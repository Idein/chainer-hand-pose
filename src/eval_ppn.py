...
def evaluate_ppn(trained, model, dataset, hand_param):
    ...
    auc = np.trapz(y=ps, x=np.linspace(min_threshold, max_threshold, n_plots))
    auc = auc / (100 * (max_threshold - min_threshold))
    print("Area under curve", auc)

    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.set_xlabel("Distance threshold / pixel")
    ax.set_ylabel("PCK [%]")
    ax.set_ylim(0, 100)
    ax.set_xlim(0, max_threshold)
    ax.plot(np.linspace(min_threshold, max_threshold, n_plots), ps)
    ax.grid(True, linestyle="--")
    plt.savefig(os.path.join(trained, "pose", "plot_PCK_ppn2D.png"))
    ...
    ax.plot(np.linspace(min_threshold, max_threshold, n_plots), ps)
    ax.grid(True, linestyle="--")
    plt.savefig(os.path.join(trained, "pose", "plot_PCK_ppn3D.png"))
