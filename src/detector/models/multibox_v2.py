...
class DepthWise_Conv(chainer.link.Chain):
    def __init__(self, in_channels, out_channels, ksize, stride):
        ...
        with self.init_scope():
            self.depthwise_conv = L.Convolution2D(in_channels, out_channels, ksize, stride, pad=1, nobias=True
                                                  , groups=in_channels)
...
class MB_module(chainer.link.Chain):

    def __init__(self, in_channels, out_channels, ksize):
        super(MB_module, self).__init__()
        with self.init_scope():
            self.depthwise = DepthWise_Conv(in_channels, in_channels, ksize, 1)
            self.pointwise = PointWise_Conv(in_channels, out_channels, 1, 1)
...
class Multibox(chainer.Chain):
    ...
    def __init__(
            self, n_class, aspect_ratios, feature_channel,
