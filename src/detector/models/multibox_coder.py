...
class MultiboxCoder(object):
    ...
    * :obj:`mb_conf`: An array representing classes of \
    ...
    def encode(self, bbox, label, iou_thresh=0.5):
        ...
        #print(self._default_bbox.shape) # (#anchor box, 4)
        #print(bbox.shape) # (#objects, 4)
        #print(iou.shape) # (#anchor box, #objects)
        #print(iou.shape)
        ...
        masked_iou = iou.copy()
        tmp_iou = xp.zeros(iou.shape)
        while True:
            # anchor box に iou が最も高いオブジェクトを割り当てる(anchor box と object は 1:1 対応)
        ...
        # 閾値以上の iou を持つ object の中で最大のものを anchor box に割り当てる
        mask = xp.logical_and(index < 0, iou.max(axis=1) >= iou_thresh)
        index[mask] = iou[mask].argmax(axis=1)
        tmp_iou[mask, iou[mask].argmax(axis=1)] = 1

        n_anchor = tmp_iou.sum(axis=0)[tmp_iou.max(axis=0)>0].mean()

        masked_iou = iou.copy()
        #masked_iou = masked_iou[tmp_iou.max(axis=1)==0, tmp_iou.sum(axis=0)< n_anchor]
        masked_iou[tmp_iou.max(axis=1)>0, :] = 0
        masked_iou[:, tmp_iou.sum(axis=0) > n_anchor] = 0

        while True:
            i, j = _unravel_index(masked_iou.argmax(), masked_iou.shape)
            if masked_iou[i, j] < 0.1:
                break
            if tmp_iou[:, j].sum() <= n_anchor:
                index[i] = j
                masked_iou[i, :] = 0
                tmp_iou[i, j] = 1
            else:
        ...
        #print(mask.shape) (#anchor box, )
        #print(index.shape) (#anchor box, )
