...
from detector.evaluation.eval_detection_coco import eval_detection_coco
from chainercv.utils.iterator import apply_to_iterator
    ...
    * :obj:`'ap/<label_names[l]>'`: Average precision for class \
    ...
    Args:
        iterator (chainer.Iterator): An iterator. Each sample should be
            ...
            will be ignored from detector.evaluation.
    ...
    def __init__(
            self, iterator, target, device,  label_names=None):
        super(DetectionCOCOEvaluator, self).__init__(
            iterator, target, device)
