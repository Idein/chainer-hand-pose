...
from detector.hand_dataset.utils import pairwise
from detector.hand_dataset.visualizations import BASE_COLOR as _BASE_COLOR
...
def wrist2palm(joint):
    ...
    j_wrist = joint[wrist_idx]
    j_midmcp = joint[middle_mcp_idx]
    parm = (joint[wrist_idx] + joint[middle_mcp_idx]) / 2
    joint[wrist_idx] = parm
    return joint


def palm2wrist(joint):
    palm_idx = STANDARD_KEYPOINT_NAMES.index("root")
    middle_mcp_idx = STANDARD_KEYPOINT_NAMES.index("middle_mcp")
    j_wrist = joint[palm_idx]
    j_midmcp = joint[middle_mcp_idx]
    vec = joint[palm_idx] - joint[middle_mcp_idx]
    joint[palm_idx] = joint[palm_idx] + vec
    return joint
